...
def train(model, train_loader, epochs, optimizer, criterion, device):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader, 1):
            ...
            optimizer.zero_grad() # zero accumulated gradients. This resets the gradients after every batch. 
            # get output of SimpleNet
            output = model(data) #forward propagate the inputs to get the model predictions. shape: [batch_size]
            # calculate loss and perform backprop
            loss = criterion(output, target) #compute the loss to check how far off the model predictions are from the correct outputs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_dim', type=int, default=10, metavar='N',
                        help='Size of hidden layer(s) (default: 10)')
